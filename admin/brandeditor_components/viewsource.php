...
if (strpos($html, '{{ADDTO}}')!==false) {
    $html=$existingRow['description'].$html;
        ...
        $stmt = $database->query("UPDATE bg_company_attributes SET description = :description, modify_dt = :modify_dt WHERE attribute_id = :attribute_id", ['description' => $html, 'modify_dt' => date("Y-m-d H:i:s"), 'attribute_id' => $existingRow['attribute_id']]);
    } else {
        $stmt = $database->query("INSERT INTO bg_company_attributes (company_id, type, description, create_dt, modify_dt) VALUES (:company_id, :type, :description, :create_dt, :modify_dt)", ['company_id' => $company_id, 'type' => 'signup_html', 'description' => $html, 'create_dt' => date("Y-m-d H:i:s"), 'modify_dt' => date("Y-m-d H:i:s")]);
    }
}
...
$sourceType='DB';
$stmt = $database->query("SELECT description FROM bg_company_attributes WHERE company_id = :company_id AND type='signup_html'", ['company_id' => $company_id]);
...
#$sourceCode = preg_replace("/(id\\1=.{0,100}\\1[^'\\\"]*\\1/i", '<span class="highlightyellow">$1</span>', $sourceCode);
...
?>
   <script>
    ...
    <hr>
        ...
        <hr>
    <div class="content" id="brandeditor_viewsource_content">

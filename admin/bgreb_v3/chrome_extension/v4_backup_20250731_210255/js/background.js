...
function fetchData( userId, aid, bid) {
  let url;
  console.log('BGBGJS||fetchData called with:', {  userId, aid, bid });

  // Determine the correct domain based on userId
  let domain = 'dev.birthday.gold';
  if (userId === -1 || userId === '-1') {
    domain = 'dev7.birthday.gold';
  }

  if (!userId) {
    console.warn('BGBGJS||Warning: No userId provided to fetchData');
    url = `https://${domain}/admin/bgreb_v3/bgr_getprocessdetails.php?uid=20&type=bgrab`;
  } else {
    url = `https://${domain}/admin/bgreb_v3/bgr_getprocessdetails.php?aid=${aid}&uid=${userId}&bid=${bid}&type=bgrab`;
  }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'userSelected') {
    console.log('BGBGJS||userSelected message received with:', {
      ...
      bid: message.bid  // Add logging for bid
    });
    
    // Modify fetchData call to be explicit about parameters
    const userIdToUse = message.userId;
    const aidToUse = message.aid;
    const bidToUse = message.bid;  // Add bid
    console.log('BGBGJS||Calling fetchData with:', { userIdToUse, aidToUse, bidToUse });
    
    fetchData(userIdToUse, aidToUse, bidToUse);  // Add bid parameter

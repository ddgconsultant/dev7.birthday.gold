...
foreach ($companies as $company) {
    ...
    // Fetch rewards for this company
    ...
    if ($rewards) {
        foreach ($rewards as $reward) {
            ...
            <div id="collapse' . $reward['reward_id'] . '" class="accordion-collapse collapse" aria-labelledby="heading' . $reward['reward_id'] . '" data-bs-parent="#businessAccordion">
            <div class="accordion-body">';
            
            echo '<h3>REWARD ID: ' . $reward['reward_id'] . '</h3>';
            
            $urls = ['COMPANY'=>$company['company_url'], 'INFO'=>$company['info_url'], 'SIGNUP'=>$company['signup_url']];
            ...
            <a href="manual_rewards?action=addnew&bid=' . $company_id . '" class="btn btn-sm btn-secondary me-5 ">Add New Reward</a>
            ...
            foreach ($textAreaArray as $field=>$code) {
                ...
                echo '  
                const textarea_' . $reward["reward_id"] . $field . ' = document.getElementById("' . $reward["reward_id"] . $field . '");';
                
                echo '
                if (textarea_' . $reward["reward_id"] . $field . ') {
                    const counter = document.getElementById("' . $reward["reward_id"] . $caption . '");
                    const initialLength = textarea_' . $reward["reward_id"] . $field . '.value.length;
                    counter.innerText = `${initialLength}/' . $size . '`;
                    if (initialLength > ' . $size . ') { counter.style.color = "red";  } else {   counter.style.color = "grey";   }

                    textarea_' . $reward["reward_id"] . $field . '.addEventListener("keyup", function() {
                        const length = textarea_' . $reward["reward_id"] . $field . '.value.length;    
                        counter.innerText = `${length}/' . $size . '`;              
                        if (length > ' . $size . ') { counter.style.color = "red";  } else {   counter.style.color = "grey";   }          
                    });     
                }';
            }
            echo '  

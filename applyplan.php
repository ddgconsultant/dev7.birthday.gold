...
if (!empty($giftcode) && !empty($giftuserid)) {
 ...
 $result=   $account->updateSettings($giftuserid, $params);
 $account->login($giftuserid, $sitesettings['app']['APP_AUTOLOGIN'], 'user_id');

  header('Location: /myaccount/');
#header('location: /applyplan_handler?redirect_status=succeeded');
exit;
}
...
# Convert from userregistrationdata to true record of bg users table
##get user_id while we are testing if needed
$userregistrationdata['user_id'] = $userregistrationdata['user_id'] ?? $userregistrationdata[':user_id'] ??  $account->getuserid($userregistrationdata[':email']) ?? $account->getuserid($userregistrationdata['email']) ??  null;

if ($userregistrationdata['user_id'] === null) {
  $pagemessage='<div class="alert alert-danger alert-dismissible fade show" role="alert">Unable to locate a valid Sign Up record</div>';
...
## get userrecord
	$tmpsettings['status']='*';
$userregistrationdata=$account->getuserdata($userregistrationdata['user_id'], 'user_id', $tmpsettings);
$userregistrationdata[':user_id']=$userregistrationdata['user_id'];
$userregistrationdata[':email']=$userregistrationdata['email'];
$userregistrationdata[':first_name']=$userregistrationdata['first_name'];
...
echo '
<!-- Start -->
<div class="container-xxl py-6" data-wow-delay="0.1s">

...
<script>
(function(){function a(e,s,t,n){let r=performance.now(),i=e.getAttribute("data-original")||e.textContent,o=i.includes("$"),l=i.includes("+"),c=i.includes("K");function d(u){let f=Math.min((u-r)/n,1),m=Math.floor(s+(t-s)*f),p=m;c&&m>=1e3&&(p=Math.floor(m/1e3)+"K");let g=p;o&&(g="$"+g),l&&(g=g+"+"),e.textContent=g,f<1&&requestAnimationFrame(d)}requestAnimationFrame(d)}document.addEventListener("DOMContentLoaded",function(){let e=document.querySelectorAll(".counter"),s=Array.from(e),t=!1;s.forEach(e=>e.setAttribute("data-original",e.textContent));let n=new IntersectionObserver(e=>{e.forEach(e=>{e.isIntersecting&&!t&&(t=!0,s.forEach(e=>{let s=e.textContent,t=s.includes("K")?parseInt(s.replace(/[^0-9]/g,""))*1e3:parseInt(s.replace(/[^0-9]/g,""));e.textContent=s.includes("$")?"$0":"0",setTimeout(()=>a(e,0,t,2e3),200)}))})},{threshold:.5}),r=document.querySelector(".stats");r&&n.observe(r)})})();

...
if (isset($_POST['ajax']) && $_POST['ajax'] == 'validate' && isset($_POST['code'])) {
    ...
    if ($test_mode || $ajax_test_mode) {
        $expected_code = $test_valid_codes[$ajax_test_type] ?? 'ABCD12';
        if ($submitted_code === $expected_code) {
            // Simulate successful validation
            echo json_encode(['success' => true, 'redirect' => '/myaccount/welcome']);
            exit;
        } else {
...
if (isset($_POST['ajax']) && $_POST['ajax'] == 'resend') {
    header('Content-Type: application/json');
    ...
    // Test mode resend
    if ($test_mode || $ajax_test_mode) {
        echo json_encode(['success' => true, 'message' => 'New code sent successfully (test mode).']);
        exit;
    }
...
// Get existing validation code for display
if ($test_mode && $test_autofill) {
    // Use test code for auto-fill
    $local_validationcode = $test_autofill === 'valid' ? $test_valid_codes[$test_code_type] : $test_autofill;
} elseif ($mode == 'dev' && $userregistrationdata['account_type'] != 'test') {
    $validatedata = array();
    $validatedata['rawdata'] = $userregistrationdata['email'];
    $validatedata['user_id'] = $userregistrationdata['user_id'];
    $validatedata['action'] = 'getlatest';
    
    $validationcodes = $app->getvalidationcodes($validatedata);
    if (!empty($validationcodes['mini'])) {
        $local_validationcode = $validationcodes['mini'];
    }
}
...
?>
    ...
    <div style="position: fixed; bottom: 20px; right: 20px; z-index: 1000;">
        <div style="background: rgba(0,0,0,0.8); color: white; padding: 0.5rem; border-radius: 8px; font-size: 0.7rem; margin-bottom: 0.5rem; text-align: center;">
            Test Mode Active<br>
            Valid codes:<br>
            <?php echo $test_code_type === 'numeric' ? '123456' : 'ABCD12'; ?>
        </div>
        <a href="?test=1&type=<?php echo $test_code_type === 'numeric' ? 'alphanumeric' : 'numeric'; ?>&email=<?php echo urlencode($userregistrationdata['email']); ?>" 
           style="background: #333; color: white; border: none; padding: 0.5rem 0.75rem; border-radius: 20px; font-size: 0.75rem; margin-bottom: 5px; display: block; text-decoration: none; text-align: center;">
            Toggle to <?php echo $test_code_type === 'numeric' ? 'Alphanumeric' : 'Numeric'; ?>
        </a>
        <a href="?test=1&error=1&type=<?php echo $test_code_type; ?>&email=<?php echo urlencode($userregistrationdata['email']); ?>" 
           style="background: #333; color: white; border: none; padding: 0.5rem 0.75rem; border-radius: 20px; font-size: 0.75rem; margin-bottom: 5px; display: block; text-decoration: none; text-align: center;">
            Simulate Error
        </a>
        <a href="/verify" style="background: #333; color: white; border: none; padding: 0.5rem 0.75rem; border-radius: 20px; font-size: 0.75rem; display: block; text-decoration: none; text-align: center;">
            Exit Test Mode
        </a>
    </div>
    <?php endif; ?>
    ...
    <div class="verification-container">
        <div class="verification-card" id="verificationCard">
            <div class="logo">Birthday.Gold</div>
    ...
    <script>
        class VerificationCodeInput {
            ...
            init() {
                ...
                // Auto-fill code
                const devCode = '<?php echo $local_validationcode; ?>';
                devCode.split('').forEach((char, index) => {

...
class fileuploader_ui {
    private $default_options = array(
		...
		// BIRTHDAY.GOLD-CUSTOM
		'createDir'=> false,
		'disallowedExtensions' => array(
		'htaccess', 'php', 'php3', 'php4', 'php5', 'phtml', 'inc', // PHP and config files
		'exe', 'bat', 'sh', 'cmd', 'com', 'msi', 'jar', // Executables and scripts
		'ds_store', 'lnk', 'url', 'sys', 'drv', 'inf', // System and shortcut files
		'jsp', 'asp', 'aspx', 'cer', 'csr', 'html', 'htm', 'xhtml', 'xml', // Web scripts and markup
		'js', 'vbs', 'vbe', 'jse', 'wsf', 'wsh', 'ps1', 'ps2', 'psm1', 'psd1', // Scripting files
		'reg', 'scr', 'msp', 'gadget', // Windows-related and potentially harmful files
		'pl', 'cgi', 'py', 'rb', // Other server-side scripting files (Perl, Python, Ruby)
		'sql', 'bak', 'backup', 'json', 'yaml', 'yml', // Database and configuration files
		'bin', 'dll', 'so', 'dylib', // Binary files and shared libraries
		'log', 'ini', 'conf', // Configuration and log files
		'swift', 'class', 'o', 'a', 'lib', 'c', 'cpp', // Compiled and source code files
		'apk', 'ipa', 'xap', 'deb', 'rpm', // Mobile app and package management files
		'crt', 'key', 'pem', 'der', 'cer', // Certificate and key files
		'torrent', 'iso', 'img', 'dmg' // Disk images and torrent files
		),
		// --end BIRTHDAY.GOLD-CUSTOM
...
private function validate($item = null) {
        if ($item == null) {
			...
			// BIRTHDAY.GOLD-CUSTOM
			if ($this->options['createDir'] && !is_dir($this->options['uploadDir'])) {
...
public static function resize($filename, $width = null, $height = null, $destination = null, $crop = false, $quality = 95, $rotation = 0) {
		...
		// BIRTHDAY.GOLD-CUSTOM
		$exif = [];
		// Only read EXIF data if the file is a JPEG or TIFF (formats that support EXIF)
		if (in_array(mime_content_type($filename), ['image/jpeg', 'image/tiff']) && function_exists('exif_read_data')) {
     ...
     */
		...
		switch ($type) {
			...
			case "md5hash":
				...
				// BIRTHDAY.GOLD-CUSTOM
			case "sha1hash":
				$string = sha1($item['title']);
				break;
				case "md5hashunique":
					// Generate MD5 hash and append a random number and timestamp to make it unique
					$string = md5($item['title'] . '_' . rand(1000, 9999) . '_' . time());
					break;
				
				// BIRTHDAY.GOLD-CUSTOM
				case "sha1hashunique":
					// Generate SHA1 hash and append a random number and timestamp to make it unique
					$string = sha1($item['title'] . '_' . rand(1000, 9999) . '_' . time());
					break;
			default:
				$string = $type;

...
if ($app->formposted('GET')) {
    ...
    if (!empty($current_user_data['user_id']) && !empty($_GET['pid'])) {
        ...
        if (!empty($_GET['id'])  && $current_user_data['user_id'] == $_GET['pid']) {
            ...
            $switchdata = $account->getuserdata($minor_id, 'user_id');
            if ($switchdata['feature_parent_id'] == $current_user_data['user_id']) {
                $account->logout();
                session_start();
            ...
            if ($response) {
                ...
                $session->set('parental_login_source', $current_user_data['user_id']);

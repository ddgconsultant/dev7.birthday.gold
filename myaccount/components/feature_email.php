 ...
 *
 * This component handles the setup of the feature_email for users whose product includes this feature.
 * Users can choose between:
 * 1. BDay Gold Inbox (@mybdaygold.com) - Managed email address
 * 2. Use their own existing email address
 *
 * After selection, redirects back to /myaccount/ which will then check for other features needing setup
 *
 * MODES:
 * - 'setup' (default): Full setup page for configuring the feature
 * - 'settings': Returns HTML for settings page display line item
 */

// Check if being called in settings mode
$featuremode = $featuremode ?? 'setup';

if ($featuremode === 'settings') {
    // Return settings display HTML
    $is_configured = !empty($current_user_data['feature_email']);
    ...
    $button_html = '';
    if ($is_configured) {
        $button_html = '<a href="/myaccount/mail-box#settings" class="btn btn-sm btn-outline-primary">
            <i class="bi bi-gear"></i> Configure
        </a>';
    } else {
        $button_html = '<a href="/myaccount/components/feature_email.php" class="btn btn-sm btn-primary">
    ...
    echo '
    <hr>
    <!-- Feature: Birthday Gold Inbox -->
    <div class="d-flex align-items-center justify-content-between px-4">
        <div class="d-flex align-items-center flex-grow-1">
            <h1><i class="bi bi-envelope-paper-heart"></i></h1>
            <div class="ms-4 flex-grow-1">
                <div class="small fw-bold">Birthday Gold Inbox ' . $status_badge . '</div>
                <div class="text-xs text-muted">Managed email address for all your birthday enrollments</div>
            </div>
        </div>
        <div class="ms-3">
            ' . $button_html . '
    ...
    return; // Exit early, don't display the full setup page
}

// Continue with normal setup mode
...
if (!$feature_email) {
    // Feature email is not enabled for this product - redirect back to myaccount
    session_tracking('feature_email not available for product - forwarding to myaccount');
...
if (!empty($current_user_data['feature_email'])) {
    // Already configured - redirect back to myaccount to check for other features
    session_tracking('feature_email already set - forwarding to myaccount');
    header('location: /myaccount/');
    exit;
}
...
#-------------------------------------------------------------------------------
// Get feature setup info from session
$feature_setup_info = isset($_SESSION['feature_setup_redirect']) ? $_SESSION['feature_setup_redirect'] : null;
$is_critical_feature = false;
$can_skip = true;

if ($feature_setup_info && $feature_setup_info['feature'] === 'feature_email') {
    $is_critical_feature = isset($feature_setup_info['is_critical']) ? $feature_setup_info['is_critical'] : false;
    $can_skip = !$is_critical_feature;
}

#-------------------------------------------------------------------------------
# HANDLE SKIP REQUEST (for non-critical features)
#-------------------------------------------------------------------------------
if ($app->formposted() && isset($_POST['skipFeature']) && $can_skip) {
    // User wants to skip this feature for now
    $skip_key = 'skip_feature_feature_email_' . date('Y-m-d');
    $_SESSION[$skip_key] = true;
    ...
    $sql = "INSERT INTO bg_user_attributes
            ...
            VALUES (:user_id, 'feature_skip', :name, :description, 'active', NOW(), NOW())
    ...
    $stmt->execute([
        ...
        'name' => 'feature_email_skipped',
        'description' => json_encode([
            'feature' => 'feature_email',
            'skipped_at' => date('Y-m-d H:i:s'),
            'priority' => $feature_setup_info['priority'] ?? 0,
            'reason' => 'user_choice'
        ])
    ]);

    session_tracking('feature_email skipped by user - forwarding to myaccount');
...
if ($app->formposted()) {
    ...
    switch ($emailoption) {
        ...
        case 'bginbox':
            ...
            if (!empty($inputEmail)) {
                // Ensure @mybdaygold.com is appended
                if (strpos($inputEmail, '@mybdaygold.com') === false) {
                    $inputEmail .= '@mybdaygold.com';
                }

                // Update user settings with the feature_email
                $updatefields = [
                ...
                $result = $account->updateUserProfileData($current_user_data['user_id'], ['profile_email' => $inputEmail]);
                ...
                $stmt->execute([
                    'user_id' => $current_user_data['user_id'],
                    'name' => 'feature_email_completed',
                    'description' => json_encode([
                        'feature' => 'feature_email',
                        'completed_at' => date('Y-m-d H:i:s'),
                        'option_selected' => 'bginbox',
                        'value' => $inputEmail
                    ])
                ]);

                session_tracking('successful bginbox post - forwarding to myaccount');

                // Redirect back to myaccount to check for other features needing setup
                header('location: /myaccount/');
                exit;
            }
            break;
        ...
        // ------------------------
        case 'ownemail':
            // User chose to use their own email
            // We need to mark that they made this choice (use existing email from registration)
            $existing_email = $current_user_data['email'] ?? '';

            if (!empty($existing_email)) {
                $updatefields = [
                    'feature_email' => $existing_email,
                ];
                $userid = $current_user_data['user_id'];
                $result = $account->updateSettings($userid, $updatefields);
                ...
                $stmt->execute([
                    ...
                    'description' => json_encode([
                        'feature' => 'feature_email',
                        'completed_at' => date('Y-m-d H:i:s'),
                        'option_selected' => 'ownemail',
                        'value' => $existing_email
                    ])
                ]);

                session_tracking('successful ownemail post - forwarding to myaccount');
            } else {
                // No existing email found - this shouldn't happen, but handle gracefully
                session_tracking('ownemail selected but no existing email found');
            }
    ...
    $transferpage['url'] = '/myaccount/components/feature_email.php';
    $transferpage['message'] = '<div class="alert alert-danger">Hmmm... something odd happened.</div>';
...
// Add skip button for non-critical features
if ($can_skip) {
    echo '
        <div class="mb-3">
            <form method="post" action="/myaccount/components/feature_email.php" class="d-inline">
                ' . $display->inputcsrf_token() . '
                <input type="hidden" name="skipFeature" value="1">
                <button type="submit" class="btn btn-sm btn-outline-secondary">
                    <i class="bi bi-arrow-right-circle me-1"></i>Skip for now
                </button>
            </form>
            <p class="text-muted small mt-2">You can set this up later from your account settings.</p>
        </div>';
} else {
    echo '
        <div class="alert alert-info mx-auto" style="max-width: 600px;">
            <i class="bi bi-info-circle me-2"></i>
            <strong>Required Setup:</strong> This feature must be configured before continuing.
        </div>';
}
...
<form id="emailChoiceForm" method="post" action="/myaccount/components/feature_email.php" class="mt-4">
    ' . $display->inputcsrf_token() . '
    <div class="input-group mb-3">
        <input type="text" class="form-control px-1 px-md-3 text-black fw-bold" name="inputEmail" id="inputEmail" placeholder="Choose your Email Address" value="' . $emailaddress . '">
    ...
    <div class="card-footer bg-light">
        <div>
            ...
            <label class="form-check-label btn btn-primary  px-3 emailbgshow disabled" for="bgInboxOption" id="bgInboxOption">I want this!</label>
            <!-- This is the hidden submit button triggered when "I want this!" is clicked -->
...
echo '
        ...
        <div class="col-lg-6 mb-5 account-type-card" data-target="#OWN">
          <div class="card bg-light border-0 h-100 d-flex flex-column">
            ...
            <div class="card-footer bg-light">
                <div>
                    <form id="emailChoiceForm2" method="post" action="/myaccount/components/feature_email.php">
                        ' . $display->inputcsrf_token() . '
...
<script>
    $(document).ready(function() {
        // Disable the "I want this!" button initially
        $("#bgInboxOption").addClass("disabled");

            // Check availability on page load based on the initial value of inputEmail
        checkAvailability();


        // Typing timer functionality to delay the availability check after typing stops
        var typingTimer;
        var doneTypingInterval = 700;  // Time in milliseconds
        ...
        function doneTyping() {
            checkAvailability(); // Check availability once typing is done
        }
        ...
        function checkAvailability() {
            ...
            if (username.trim() === "") {
                $("#availability").html("Please enter a valid email").css("color", "red");
                $("#bgInboxOption").addClass("disabled"); // Disable the "I want this!" button
                return;
            }
            ...
            $.post("/helper_checkavailability", {
                type: "f.email",
                username: username,
                _token: "' . $display->inputcsrf_token('tokenonly') . '"
            }, function(data) {
                $("#spinner").addClass("d-none");  // Hide the spinner
                if (data == "1") {
                    $("#availability").html("Available").css("color", "green");
                    $("#bgInboxOption").removeClass("disabled"); // Enable the "I want this!" button
                } else {
        ...
        // Form submission when the "I want this!" button is clicked

<?php
// abo_grablocations.php - Extract store locations with AIRTOP integration
...
try {
    ...
    try {
                                    ...
                                    if (waitForSessionReady($system, $airtopApiUrl, $airtopApiKey, $sessionId)) {
                                        ...
                                        if (isset($windowResponse['decoded']['data']['windowId'])) {
                                            ...
                                            $prompt = "Extract all store/location addresses from this page. For each location, provide:
                                                      ...
                                                      7. Location name (if available)
                                                      8. Location URL - if clicking on the location leads to a detail page (if available)
                                                      Return as JSON array with fields: address, city, state, zip_code, phone_number, business_hours, name, url";
                                            ...
                                            if (isset($queryResponse['decoded']['data']['modelResponse'])) {
                                                ...
                                                if ($debug) {
                                                    $debug_output['airtop_raw_response'] = substr($aiResponse, 0, 500) . '...';
                                                }
                                                    ...
                                                    foreach ($extracted_locations as $loc) {
                                                        if (!empty($loc['address']) && !empty($loc['city'])) {
                                                            $location = [
                                                                ...
                                                                'location_name' => $loc['name'] ?? ($loc['city'] . ', ' . ($loc['state'] ?? '')),
                            ...
                            if (waitForSessionReady($system, $airtopApiUrl, $airtopApiKey, $sessionId)) {
                                ...
                                if (isset($windowResponse['decoded']['data']['windowId'])) {
                                    ...
                                    if (isset($queryResponse['decoded']['data']['modelResponse'])) {
                                        ...
                                        if ($debug) {
                                            $debug_output['airtop_pattern_raw_response'] = substr($aiResponse, 0, 500) . '...';
                                        }
        ...
        if (!empty($locations_found)) {
            ...
            // Save to database
            if ($debug) {
                $debug_output['locations_to_save'] = count($unique_locations);
                $debug_output['sample_locations'] = array_slice($unique_locations, 0, 3);
            }
            ...
            foreach ($unique_locations as $idx => $location) {
                ...
                // Add state to the query if available
                if (!empty($location['state'])) {
                    $check_sql .= " AND (state = :state OR state IS NULL OR state = '')";
                    $check_params['state'] = $location['state'];
                }
                
                // Look for locations in the same city that might be the same location
                $check_sql .= " ORDER BY 
                    CASE 
                        WHEN address = :address_exact THEN 0
                        WHEN address LIKE :address_pattern THEN 1
                        ELSE 2
                    END,
                    location_id ASC";
                
                $check_params['address_exact'] = $location['address'];
                // Create a pattern that matches the street number and name, ignoring suite/unit differences
                $address_parts = preg_match('/^(\d+\s+[A-Za-z\s]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Lane|Ln|Drive|Dr|Way|Court|Ct|Plaza|Place|Pl))/', $location['address'], $matches);
                $check_params['address_pattern'] = $address_parts ? $matches[1] . '%' : $location['address'] . '%';
                
                $stmt = $database->query($check_sql, $check_params);
                $existing_location = $stmt->fetch(PDO::FETCH_ASSOC);
                
                if ($debug && $idx < 3) {
                    $debug_output['location_check_' . $idx] = [
                        'address' => $location['address'],
                        'city' => $location['city'],
                        'state' => $location['state'] ?? 'N/A',
                        'existing' => $existing_location ? 'YES' : 'NO',
                        'existing_data' => $existing_location
                    ];
                }
                
                if ($existing_location) {
                    // Update existing location with new data - check ALL fields for changes
                    $updates = [];
                    ...
                    if ($existing_location['address'] !== $location['address']) {
                        ...
                        $update_params['address'] = $location['address'];
                        $needs_update = true;
                    }
                    
                    // Check state change
                    if (($existing_location['state'] ?? '') !== ($location['state'] ?? '')) {
                        $updates[] = 'state = :state';
                        $update_params['state'] = $location['state'] ?? null;
                        $needs_update = true;
                    }
                    
                    // Check zip code change
                    if (($existing_location['zip_code'] ?? '') !== ($location['zip_code'] ?? '')) {
                        $updates[] = 'zip_code = :zip_code';
                        $update_params['zip_code'] = $location['zip_code'] ?? null;
                        $needs_update = true;
                    }
                    
                    // Check phone number change
                    if (($existing_location['phone_number'] ?? '') !== ($location['phone_number'] ?? '')) {
                        $updates[] = 'phone_number = :phone_number';
                        $update_params['phone_number'] = $location['phone_number'] ?? null;
                        $needs_update = true;
                    }
                    
                    // Check business hours change
                    if (($existing_location['business_hours'] ?? '') !== ($location['business_hours'] ?? '')) {
                        $updates[] = 'business_hours = :business_hours';
                        $update_params['business_hours'] = $location['business_hours'] ?? null;
                        $needs_update = true;
                    }
                    
                    // Check location name change
                    if (($existing_location['location_name'] ?? '') !== ($location['location_name'] ?? '')) {
                        $updates[] = 'location_name = :location_name';
                        $update_params['location_name'] = $location['location_name'] ?? null;
                        $needs_update = true;
                    }
                    
                    // Check location URL change
                    if (($existing_location['location_url'] ?? '') !== ($location['location_url'] ?? '')) {
                        $updates[] = 'location_url = :location_url';
                        $update_params['location_url'] = $location['location_url'] ?? null;
                        $needs_update = true;
                    }
                    
                    if ($debug && $idx < 3) {
                        $debug_output['location_update_check_' . $idx] = [
                            'needs_update' => $needs_update,
                            'updates' => $updates,
                            'differences' => [
                                'address' => [
                                    'old' => $existing_location['address'],
                                    'new' => $location['address'],
                                    'changed' => $existing_location['address'] !== $location['address']
                                ],
                                'state' => [
                                    'old' => $existing_location['state'] ?? 'NULL',
                                    'new' => $location['state'] ?? 'NULL',
                                    'changed' => ($existing_location['state'] ?? '') !== ($location['state'] ?? '')
                                ],
                                'zip_code' => [
                                    'old' => $existing_location['zip_code'] ?? 'NULL',
                                    'new' => $location['zip_code'] ?? 'NULL',
                                    'changed' => ($existing_location['zip_code'] ?? '') !== ($location['zip_code'] ?? '')
                                ],
                                'phone_number' => [
                                    'old' => $existing_location['phone_number'] ?? 'NULL',
                                    'new' => $location['phone_number'] ?? 'NULL',
                                    'changed' => ($existing_location['phone_number'] ?? '') !== ($location['phone_number'] ?? '')
                                ],
                                'business_hours' => [
                                    'old' => $existing_location['business_hours'] ?? 'NULL',
                                    'new' => $location['business_hours'] ?? 'NULL',
                                    'changed' => ($existing_location['business_hours'] ?? '') !== ($location['business_hours'] ?? '')
                                ]
                            ]
                        ];
                    }

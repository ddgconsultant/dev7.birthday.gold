<?php
// abo_grabterms.php - Extract terms of service/conditions and track content changes
...
try {
    ...
    if (!$company) {
        $result['message'] = 'No companies pending terms collection';
    ...
    try {
        ...
        // Possible variations for Terms and Conditions
        $terms_variations = [
        ...
        foreach ($urls_to_check as $url) {
            ...
            // Look for terms links in the HTML
            ...
            foreach ($dom->getElementsByTagName('a') as $anchor) {
                ...
                foreach ($terms_variations as $term) {
                    ...
                    if ($text_lower === $term_lower || 
                        $text_lower === 'terms' || 
                        $text_lower === 'terms of service' ||
                        $text_lower === 'terms & conditions' ||
                        preg_match('/\b' . preg_quote($term_lower, '/') . '\b/', $text_lower) ||
                        preg_match('/\/terms[^a-z]|\/terms-|\/terms$|\/tos[^a-z]|\/legal\/terms/', $href_lower)) {
            ...
            if ($terms_found) break; // Stop checking other URLs
        }
        
        // If we found a terms URL, fetch and hash the content
        if ($terms_found && !empty($terms_url)) {
            // Fetch the terms page
            ...
            $terms_html = curl_exec($ch);
            $terms_httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
            curl_close($ch);
            
            if ($terms_httpCode === 200 && !empty($terms_html)) {
                // Extract text content from HTML
                $dom = new DOMDocument();
                @$dom->loadHTML($terms_html);
                ...
                if ($body) {
                    $terms_content = $body->textContent;
                    // Clean up whitespace
                    $terms_content = preg_replace('/\s+/', ' ', trim($terms_content));
                    
                    // Generate content hash for change detection
                    $content_hash = hash('sha256', $terms_content);
                }
            }
        }
        ...
        if ($terms_found && !empty($terms_url)) {
            ...
            $url_sql = "INSERT INTO bg_company_attributes 
                       ...
                       VALUES 
                       (:company_id, 'url', 'terms', :url, 'active', 'policies', NOW())
            ...
            if (!$existing_policy) {
                ...
                $policy_sql = "INSERT INTO bg_company_policies 
                             ...
                             VALUES 
                             (:company_id, 'terms', 'Terms of Service', :url, :hash, 
                              1, 'verified', NOW())";
            ...
            $status = 'completed';
            $result['successful'] = 1;
            $result['data_collected'] = 'Terms found at: ' . $terms_url;
            $result['content_hash'] = $content_hash;
        } else {
            // No terms found
            $status = 'attempted';
            $result['successful'] = 1;
            $result['data_collected'] = 'No terms and conditions found';
            
            // Log the attempt
            $attr_sql = "INSERT INTO bg_company_attributes 
                        (company_id, type, name, description, status, create_dt)
                        VALUES 
                        (:company_id, 'data_collection', 'terms_search_result', 'not_found', 'active', NOW())";
        ...
        session_tracking('ABO grab terms error', "Company $company_id: " . $e->getMessage());
    }
    ...
    session_tracking('ABO grab terms fatal error', $e->getMessage());
}

...
SELECT  
    `category`,
    `name` AS Holiday,
    `content` AS `HolidayDetails`,
    `description` AS `HolidayDate`,
    null AS `Current Year`,
    null AS `Next Year`,
    'No' AS `HolidayIn24Hrs`,
    'No' AS `UpcomingHoliday`,
    'Yes' AS ProcessPDO
FROM 
    `birthday_gold_www`.`bg_content` 
WHERE 
    `type` LIKE 'calendar_personal'
...
SELECT * FROM (
    ...
    FROM 
        `bg_content`
    WHERE 
        (`type` LIKE 'calendar_%' AND `type` != 'calendar_personal')
        AND (
...
foreach ($holidayResults as $holiday) {
    ...
    $isPersonalDayOff = ($holiday['ProcessPDO'] === 'Yes');
    ...
    foreach ($staffResults as $staff) {
        ...
        $userchathandle = $account->getUserAttribute($staff['user_id'], $attribute, '', 'description');

        if ($userchathandle) {
            $channel = '@' . $userchathandle;
            ...
            try {
                ...
                // Only proceed if it's 6 AM in the user's local time
                if ($currentHourUser === 6) {
...
// =================================================================
                    ...
                    } elseif ($isUpcomingHoliday) {
                        // =================================================================
                        // If the holiday is within the next $upcomingdays days, send an upcoming holiday message
                        $formattedHolidayDate = $holidayDate->format('F j, Y'); // Example: "October 11, 2024"
...
// =================================================================
                    // Check if today is Monday and ProcessPDO is true
if (date('N') == 1 && $isPersonalDayOff) {  // date('N') == 1 checks if today is Monday
    $qik->logmessage("Today is Monday, checking special date for {$staff['username']}...");

    // Get the current user's special date (e.g., birthdate, hiredate, anniversary)
    $attribute = $holiday['HolidayDate'];  // Use type and name
        $specialdate_results = $account->getUserAttribute($staff['user_id'], $attribute, '', 'description');

        foreach( $specialdate_results as $specialdate) {;  // Assuming you have 'specialdate' like 'birthdate', 'hiredate', etc.
    
    // If specialdate exists, proceed
    if ($specialdate) {
        $currentYear = date('Y');  // Get the current year
        $specialDateCurrentYear = new DateTime($specialdate); // Assuming $specialdate is in 'YYYY-MM-DD' format
        $specialDateCurrentYear->setDate($currentYear, $specialDateCurrentYear->format('m'), $specialDateCurrentYear->format('d'));  // Set specialdate to the current year

        // Get the current date and calculate the date range for the next week (Monday to Sunday)
        $currentDate = new DateTime('now', $userTimeZoneObj);
        $nextWeekStart = clone $currentDate;
        $nextWeekStart->modify('next Monday')->setTime(0, 0, 0);  // Next week's Monday
        $nextWeekEnd = clone $nextWeekStart;
        $nextWeekEnd->modify('+6 days')->setTime(23, 59, 59);  // Next week's Sunday

        $qik->logmessage("Checking if {$staff['username']}'s special date ({$specialDateCurrentYear->format('Y-m-d')}) falls within next week from {$nextWeekStart->format('Y-m-d')} to {$nextWeekEnd->format('Y-m-d')}...");

        // Check if the special date is within next week's date range
        if ($specialDateCurrentYear >= $nextWeekStart && $specialDateCurrentYear <= $nextWeekEnd) {
            $formattedSpecialDate = $specialDateCurrentYear->format('F j');  // Format the special date (e.g., "October 11")
            $personalMessage = "Hi {$staff['first_name']}, your {$holidayName} is coming up on {$formattedSpecialDate}. Don't forget to celebrate!";
            $qik->logmessage("Sending special date reminder for {$staff['username']}...");

            // Post the message to Rocket.Chat
            $system->postToRocketChat($personalMessage, $channel);
        } else {
            $qik->logmessage("Special date does not fall within next week for {$staff['username']}.");
        }
    } else {
        $qik->logmessage("No special date found for {$staff['username']}.");
    }
}
} else {
    $qik->logmessage("Today is not Monday or this is not a personal day off (ProcessPDO).");
}
                    ...
                    // Log if it's not the 6 AM hour
                    $qik->logmessage("Not 6 AM for {$staff['username']}, no message sent.");
                }
            } catch (Exception $e) {
